...
for tags in bs_info.find_all('div',attrs={'class':'movie-hover-info'}):
    for tag in tags.find_all('div',attrs={'class':'movie-hover-title'}):
        for i in tag.find_all('span',attrs={'class':'hover-tag'}):
            if i.text == '类型:':
                movie_category = parse_text(i.find_parent('div').text)  
            if i.text == '上映时间:':
...
result = pd.DataFrame(movie_infos[:10],columns=['movie-title', 'movie-category', 'movie-date'])
result.to_csv('./movie1.csv',encoding='gbk',index=False,header=False)
